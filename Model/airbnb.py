...
airbnb = pd.read_csv('Airbnb_U4.csv',
...
airbnb['price'] = round(np.exp(airbnb['log_price']),1)
...
airbnb.drop(airbnb[ airbnb['price'] < 20 ].index , inplace=True)
airbnb.drop(airbnb[ airbnb['price'] > 1500 ].index , inplace=True)
airbnb.drop(airbnb[ airbnb['beds'] == 0 ].index , inplace=True)
airbnb.drop(airbnb[ airbnb['bedrooms'] == 0 ].index , inplace=True)


# MACHINE LEARNING
# Define X & y

X_train = airbnb.drop(columns=['log_price', 'price'])
